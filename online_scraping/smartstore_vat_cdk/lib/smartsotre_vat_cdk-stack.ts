...
export class SmartsotreVatCdkStack extends cdk.Stack {
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        const lambdaFn = new lambda.DockerImageFunction(this, 'SmartstoreVatLambda', {
            code: lambda.DockerImageCode.fromImageAsset(path.join(__dirname, '../../lambda'), {
                file: 'Dockerfile_smartstore_vat',
                buildArgs: {
                    IS_DOCKER: 'true',
                },
            }),
            functionName: `SmartstoreVatFunction-${stageName}`,
        ...
        const api = new apigateway.LambdaRestApi(this, 'SmartstoreVatApi', {
            handler: lambdaFn,
            proxy: true,
            deployOptions: {
                stageName: stageName,
