...
interface CoupangSubAccountVatCdkStackProps extends cdk.StackProps {
  stageName: string;
}

export class CoupangSubAccountVatCdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: CoupangSubAccountVatCdkStackProps) {
    ...
    const lambdaFn = new lambda.DockerImageFunction(this, 'CoupangSubAccountVatLambda', {
      code: lambda.DockerImageCode.fromImageAsset(path.join(__dirname, '../../lambda'), {
        file: 'Dockerfile_coupang_sub_account_vat',
        buildArgs: {
          IS_DOCKER: 'true',
        },
      }),
      functionName: `CoupangSubAccountVatFunction-${stageName}`,
    ...
    const api = new apigateway.LambdaRestApi(this, 'CoupangSubAccountVatApi', {
      handler: lambdaFn,
      proxy: true,
      deployOptions: {
        stageName: stageName,
